...
import '../../../utils/head_balance_view.dart';

final key = new GlobalKey<_TicketLotoState>();

class TicketLoto636View extends StatefulWidget {
  const TicketLoto636View({Key? key, this.code}) : super(key: key);

  final String? code;

  @override
  State<TicketLoto636View> createState() => _TicketLotoState();
}

class _TicketLotoState extends State<TicketLoto636View> {
  ...
  getLotoDraw() async {
    ...
    ResponseObject res = await _con.getDrawLoto636();
  ...
  String random(List<String> _listBall) {
    var rng = Random();
    String s = (rng.nextInt(36) + 1).toString().padLeft(2, '0');
  ...
  next() async {
    ...
    if (mode != Common.ANDROID_MODE_UPLOAD ||
      ...
      return dialogQR(context,
          "${_line.join("|")}|$drafAmount|${getProductName(Common.ID_DIENTOAN_636)}");
    }
    ...
    order.productID = Common.ID_DIENTOAN_636;
    order.quantity = 1;
    ...
    order.productDT = _system;

    OrderAddItemRequest item = new OrderAddItemRequest();
    item.productID = Common.ID_DIENTOAN_636;
...
class _LineView extends State<LineView> {
  ...
  List<String> balss =
      List.generate(36, (index) => (index + 1).toString().padLeft(2, '0'));
