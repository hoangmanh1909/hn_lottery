...
dialogPayment(
  ...
  onOk() async {
    TransPaymentRequest req = TransPaymentRequest(
        fee: orderAddNewRequest.fee,
        orderCode: code,
        mobileNumber: profile.mobileNumber);
    if (context.mounted) {
      showProcess(context);
    }
    ResponseObject res = await con.payment(req);
    if (context.mounted) Navigator.pop(context);
    if (res.code == "00") {
      if (context.mounted) {
        if (context.mounted) Navigator.pop(context);
        if (orderAddNewRequest.productID == Common.ID_DIENTOAN_636 ||
            orderAddNewRequest.productID == Common.ID_LOTO234 ||
            orderAddNewRequest.productID == Common.ID_LOTO235) {
          preferences!.setString(Common.MAIN_INDEX, "1");
        } else {
